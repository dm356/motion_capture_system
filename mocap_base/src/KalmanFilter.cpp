...
namespace mocap {
  ...
  void KalmanFilter::estimatePosition(const double& curr_time_stamp, Eigen::Vector3d& p_position) const
  {
    ...
    p_position = dr + position;
  }

  void KalmanFilter::estimateAttitude(const double& curr_time_stamp, Eigen::Quaterniond& p_attitude) const
  {
    // Propogate the actual state
    double dt = curr_time_stamp - last_time_stamp;
    //dt = dt > 0 ? dt : msg_interval;
